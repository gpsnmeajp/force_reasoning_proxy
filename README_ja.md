...
9. **チャンク途絶タイムアウト**: 推論中・生成中に意味のあるチャンクが一定時間（デフォルト **10 秒**）届かない場合、生成をキャンセルしてリトライします。空チャンクが流れ続けるケースにも対応します（`--chunk-timeout` で変更可能、0 以下で無効）。
10. **Reasoning タイムアウト**: Reasoning フェーズが一定時間（デフォルト **600 秒**）を超えた場合、生成をキャンセルしてリトライします（`--reasoning-timeout` で変更可能、0 以下で無効）。
11. **生成タイムアウト**: コンテンツ生成フェーズが一定時間（デフォルト **600 秒**）を超えた場合、生成をキャンセルしてリトライします（`--generation-timeout` で変更可能、0 以下で無効）。
...
| `--chunk-timeout` | `10`                      | チャンク途絶タイムアウト（秒）。意味のあるチャンクがこの秒数届かない場合にリトライ（0 以下で無効） |
| `--reasoning-timeout` | `600`               | Reasoning タイムアウト（秒）。Reasoning フェーズがこの秒数を超えた場合にリトライ（0 以下で無効） |
| `--generation-timeout` | `600`              | 生成タイムアウト（秒）。コンテンツ生成フェーズがこの秒数を超えた場合にリトライ（0 以下で無効） |
